...
@section('title', 'List service option full tour')
...
@section('content')
    <div class="content-wrapper">
        ...
        {{-- @include('admin.partials.content-header', [
            'name' => 'Service option',
            'key' => 'List service option full tour',
        ]) --}}
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <form action="{{ route('admin.service-full-option.saveInformation') }}" method="post">
                            @csrf
                                            ...
                                            <div class="box-service-option title">
                                                ...
                                                <div class="d-flex">
                                                    <button class="btn btn-warning mx-2 trigger-modal-btn"
                                                        ...
                                                        data-url="{{ route('admin.service-full-season.update', ['service_full_season' => $item->id]) }}">Update</button>
                                                    <button class="btn btn-danger delete-record"
                                                        data-url="{{ route('admin.service-full-season.destroy', ['service_full_season' => $item->id]) }}"
                                                    ...
                                                    <tbody>
                                                        @foreach ($item->services()->orderBy('created_at', 'desc')->get() as $index => $option)
                                                            <tr>
                                                                ...
                                                                <td>{{ $option->type }}</td>
                                                                <td>
                                                                    <a href="{{ route('admin.service-full-option.edit', ['service_full_option' => $option->id]) }}"
                                                                        class="btn btn-warning delete-record-option">Edit</a>
                                                                    <button
                                                                        data-url="{{ route('admin.service-full-option.destroy', ['service_full_option' => $option->id]) }}"
                                                                        data-method="DELETE"
                                                        ...
                                                        <tr>
                                                            <td colspan="5" style="cursor: pointer">
                                                                <a
                                                                    href="{{ route('admin.service-full-option.create', ['service_id' => $service->id, 'service_season_id' => $item->id]) }}">
    ...
    <div class="custom-modal-overlay" id="custom-modal">
        <form id="customForm" action="{{ route('admin.service-full-season.store', ['service_id' => $service->id]) }}"
