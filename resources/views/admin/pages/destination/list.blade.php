                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="text-right">
                            <a href="{{ route('admin.destination.create', ['parent_id' => request()->parent_id ?? 0]) }}"
                                class="btn  btn-info btn-md mb-2">+ Thêm mới</a>
                        </div>
                        <div class="card card-outline card-info">
                            <div class="card-header pt-2 pb-2">
                                <div class="cart-title">
                                    <i class="fas fa-list"></i> Điểm đến
                                </div>
                            </div>
                        </div>
                        ...
                        @if (isset($parentBr) && $parentBr)
                            <ol class="breadcrumb">
                                <li><a href="{{ route('admin.destination.index', ['parent_id' => 0]) }}">Root</a></li>

                                @foreach ($parentBr->breadcrumb as $item)
                                    <li><a
                                            href="{{ route('admin.destination.index', ['parent_id' => $item['id']]) }}">{{ $item['name'] }}</a>
                                    </li>
                                @endforeach

                                <li><a
                                        href="{{ route('admin.destination.index', ['parent_id' => $parentBr->id]) }}">{{ $parentBr->name }}</a>
                                </li>
                            </ol>
                        @endif
                                        ...
                                        <li class="lb_item_recusive font-weight-bold  lb_item_delete  border-bottom">
                                            <div class="d-flex">
                                                <div class="box-left lb_list_content_recusive ">
                                                    <div class="d-flex">
                                                        ...
                                                        <div class="col-sm-2  pt-2 pb-2 parent text-center wrap-load-hot" 
                                                            data-url="{{ route('admin.destination.load.hot', ['id' => $value->id]) }}">
                                                            @include('admin.components.load-change-hot',['data'=>$value,'type'=>'destination'])
                                                         </div>
                                                        <div class="col-sm-2 pt-2 pb-2 slug text-center">
                                                            <input
                                                                data-url="{{ route('admin.loadOrderVeryModel', ['table' => 'destinations', 'id' => $value->id]) }}"
                                                ...
                                                <div class="pt-1 pb-1 lb_list_action_recusive">
                                                    <a href="{{ route('admin.destination.edit', ['id' => $value->id]) }}"
                                                        class="btn btn-sm btn-info"><i class="fas fa-edit"></i></a>
                                                    <a href="{{ route('admin.destination.create', ['parent_id' => $value->id]) }}"
                                                        class="btn btn-sm btn-info">+ Thêm</a>
                                                    <a data-url="{{ route('admin.destination.destroy', ['id' => $value->id]) }}"
