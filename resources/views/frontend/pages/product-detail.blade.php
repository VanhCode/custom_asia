...
@section('content')
            ...
            <div class="slide-3">
                @foreach ($data->images as $item)
                    <div class="tour-detail-slide-box clm">
                        <a href="{{ asset($item->image_path) }}" data-fancybox="img">
    ...
    <div class="tour-detail-top">
        <div class="ctnr">
            <h1 class="tour-detail-title">{{ $data->name }}</h1>
            <ul class="d-flex ai-center">
                ...
                @if ($data->number)
                    <li>
                        Duration: {{ $data->number }} days
                    </li>
                @endif
                @if ($data->price)
                    <li>
                        Pricing: ${{ number_format($data->price) }}
                    </li>
                @endif
            </ul>
        </div>
            ...
            <div class="row">
                <div class="clm" style="--w-lg: 8; --w-xs:12;">
                    <div class="tour-detail-content">
                        ...
                        {{-- @if ($data->file)
                            <div class="">
                                <a href="{{ asset($data->file2) ?? asset($data->file) }}" data-fancybox="img-map">
                                    <img src="{{ asset($data->file) }}" alt="Image" id="image">
                                </a>
                            </div>
                        @endif --}}
                        ...
                        @if ($data->file2)
                            ...
                            <div class="overlay" id="overlay">
                                <img src="{{ asset($data->file2) }}" alt="Zoomed Image" id="zoomedImage">
                                <!-- Bắt đầu với src rỗng -->
                            </div>
                        @endif --}}
                            ...
                            <div class="tour-detail-content_box">
                                ...
                                <div class="p-relative">
                                    ...
                                    {{-- <div class="map">
                                        ...
                                        <ul class="list-destination d-none" id="list-address">
                                            @if (isset($dataMaps))
                            ...
                            <div class="tour-detail-content_box faqs--js">
                                ...
                                <div class="faqs">
                                    @foreach ($data->tabs as $key => $item)
                                        <div
                                            class="faqs-container p-relative @if ($key == 0) active @endif">
                                            ...
                                            <div class="faqs-top d-flex js-between">
                                                <a class="faqs-title" tabindex="0">
                                                    <span style="color:#d57c48">{{ $item->name }}</span>
                                                    {{ $item->description }}
                                                </a>
                        ...
                        @if (isset($downloadPDF))
                            <div class="tour-detail-content_box tour-detail-download d-flex ai-center">
                                <div class="flex-1">
                                    <h2 class="tour-detail-content_title">{{ $downloadPDF->name }}</h2>
                                    <span>{{ $downloadPDF->value }}</span>
                                </div>
                                <a href="{{ $downloadPDF->slug }}" target="_blank" class="toggle-link">
                                    <img src="{{ asset($downloadPDF->image_path) }}" alt="{{ $downloadPDF->name }}">
                                </a>
                            </div>
                        @endif
                        ...
                        @if (isset($question))
                            <div class="tour-detail-content_box faqs--js">
                                <h2 class="tour-detail-content_title">{{ $question->name }}</h2>
                                <div class="faqs faqs--sc">
                                    @foreach ($question->childs()->where('active', 1)->orderBy('order')->get() as $key => $item)
                                        <div
                                            class="faqs-container faqs-container--sc p-relative @if ($key == 0) active @endif">
                                            <div class="faqs-top d-flex js-between">
                                                <a class="faqs-title" tabindex="0">
                ...
                <div class="clm" style="--w-lg: 4; --w-xs:12;">
                    <div class="form-book">
                        <form action="{{ route('contact.planTrip') }}" id="contactTour" method="POST">
                            @csrf
    ...
    @if (isset($dataRelate) && count($dataRelate) > 0)
    ...
    <div id="pdf_popup" class="scout-component__modal js-brochure-modal__overlay--form">
        <div class="js-scout-component__modal-dialog scout-component__modal-dialog">
            <div class="scout-component__modal-top">
                ...
                <h3 class="scout-component__modal-heading"> {{ $titleForm->name ?? 'Download Brochure' }}
                </h3>
            </div>
            <div class="scout-component__modal-content">
                <div class="brochure-modal__wrap">
                    <form action="{{ route('contact.storeAjax') }}" data-url="{{ route('contact.storeAjax') }}"
                        ...
                        <input type="hidden" name="title" value="{{ $titleForm->name ?? 'Download Brochure' }}">
                        @csrf

                        <input type="hidden" name="tour_name" value="{{ $data->name }}">

                        <p class="brochure-modal__text"> {!! $titleForm->value ?? 'Send all details for the' !!} ‘
                            <span class="js-tour-name">{{ $data->name }}</span>’
