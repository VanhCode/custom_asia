...
class AdminAttributeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('parent_id')) {
            $data = $this->attribute->where('parent_id', $request->input('parent_id'))->orderBy('order')->orderBy("id", "desc")->paginate(15);
            if ($request->input('parent_id')) {
                $parentBr = $this->attribute->find($request->input('parent_id'));
            }
        } else {
            $data = $this->attribute->where('parent_id', 0)->orderBy('order')->orderBy("id", "desc")->paginate(15);
        }
        return view(
    ...
    public function store(ValidateAddAttribute $request)
    {
        try {
            ...
            $dataUploadImage = $this->storageTraitUpload($request, "image", "attribute");
            if (!empty($dataUploadImage)) {
                $dataAttributeCreate["image"] = $dataUploadImage["file_path"];
            }
    ...
    public function update(ValidateEditAttribute $request, $id)
    {
        try {
            ...
            $dataUpdateAvatar = $this->storageTraitUpload($request, "image", "attribute");
            if (!empty($dataUpdateAvatar)) {
                $path = $this->attribute->find($id)->image;
                if ($path) {
                    Storage::delete($this->makePathDelete($path));
                }
                $dataAttributeUpdate["image"] = $dataUpdateAvatar["file_path"];
            }
