...
class AdminProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->input('category')) {
            ...
            $idCategorySearch = $this->categoryProduct->getALlCategoryChildrenAndSelf($categoryProductId);
            $id_prd = ProductCate::whereIn('category_id', $idCategorySearch)->pluck('product_id')->toArray();
            //            $idCategorySearch[] = (int)($categoryProductId);
            $data = $data->whereIn('id', $id_prd);
    ...
    public function show($id)
    {
        ...
        $data_locations = DB::table('city_location')->get();
        $results = [];
        foreach ($data_locations as $item) {
            $results[] = [
                'id' => $item->id,
        ...
        $dataMaps = DB::table('location_tours')->where('tour_id', $id)->get();
        $categoryAttrOfAdmin = ProductAttribute::where('product_id', $data->id)->get()->pluck('attribute_id');

        $html = view("admin.components.product", [
            'data_ed' => $data_ed,
            'post' => $post,
    ...
    public function loadActive($id)
    {
        ...
        if ($updateResult) {
            return response()->json([
                "code" => 200,
                "html" => view('admin.components.load-change-active', ['data' => $product, 'type' => 'tour'])->render(),
                "message" => "success"
            ], 200);
        } else {
            return response()->json([
    ...
    public function loadHot($id)
    {
        ...
        if ($updateResult) {
            return response()->json([
                "code" => 200,
                "html" => view('admin.components.load-change-hot', ['data' => $product, 'type' => 'tour'])->render(),
                "message" => "success"
            ], 200);
        } else {
            return response()->json([
    ...
    public function store(ValidateAddProduct $request)
    {
        try {
            ...
            $dataProductCreate = [
                ...
                "file" => $request->file ? parse_url($request->file, PHP_URL_PATH) : '',
                "file2" => $request->file2 ? parse_url($request->file2, PHP_URL_PATH) : '',
            ];
            ...
            if ($request->gallary) {
                $galaries = explode(',', $request->gallary);


                $galaries = array_map(function ($item) {
                    return [
                        "name" => parse_url($item, PHP_URL_PATH),
                        "image_path" => parse_url($item, PHP_URL_PATH),
                        "order" => 0
                    ];
                }, $galaries);
                ...
                $product->images()->createMany($galaries);
            }
            ...
            DB::commit();
            return redirect()->route('admin.product.index')->with("alert", "Thêm tour thành công");
        } catch (\Exception $exception) {
            DB::rollBack();
            Log::error('message' . $exception->getMessage() . 'line :' . $exception->getLine());
            return redirect()->route('admin.product.index')->with("error", "Thêm tour không thành công");
        }
    }
    ...
    public function update(ValidateEditProduct $request, $id)
    {
        try {
            ...
            if ($request->images_old) {
                ...
                if (count($product->images) > 0) {
                    foreach ($product->images as $key => $image) {
                        if (!in_array($image->image_path, $request->images_old)) {
                            Storage::delete($this->makePathDelete($image->image_path));
                            $image->delete();
                        }
                    }
                }
            ...
            // $dataUploadAvatar = $this->storageTraitUpload($request, "avatar_path", "product");
            // if (!empty($dataUploadAvatar) || !$request->avatar_path_old) {
            //     $path = $this->product->find($id)->avatar_path;
            //     if ($path) {
            //         Storage::delete($this->makePathDelete($path));
            //     }
            //     $dataProductUpdate["avatar_path"] = $dataUploadAvatar["file_path"] ?? null;
            // }

            // $dataUploadFile = $this->storageTraitUpload($request, "file", "file");
            // if (!empty($dataUploadFile) || !$request->file_old) {
            //     $path = $this->product->find($id)->file;
            //     if ($path) {
            //         Storage::delete($this->makePathDelete($path));
            //     }
            //     $dataProductUpdate["file"] = $dataUploadFile["file_path"] ?? null;
            // }

            // $dataUploadFile2 = $this->storageTraitUpload($request, "file2", "file");
            // if (!empty($dataUploadFile2) || !$request->file2_old) {
            //     $path = $this->product->find($id)->file2;
            //     if ($path) {
            //         Storage::delete($this->makePathDelete($path));
            //     }
            //     $dataProductUpdate["file2"] = $dataUploadFile2["file_path"] ?? null;
            // }
            ...
            if ($request->has("category")) {
                ...
                $product->productscate()->sync($category_ids);
            }
            ...
            // insert attribute to product
            if ($request->has('attribute') && is_array($request->input('attribute'))) {
                $attribute_ids = [];
                foreach ($request->input('attribute') as $attributeItem) {
                    if (is_array($attributeItem)) {
                        foreach ($attributeItem as $attr) {
                            $attributeInstance = $this->attribute->find($attr);
            ...
            return redirect()->route('admin.product.index', ['page' => session('page')])->with("alert", "Sửa tour thành công");
        } catch (\Exception $exception) {
            //throw $th;
            dd($exception);
            DB::rollBack();
            ...
            return redirect()->route('admin.product.index', ['page' => session('page')])->with("error", "Sửa tour không thành công");
        }
    }
    ...
    public function saveMap(Request $request)
    {
        try {
            ...
            $check = DB::table('location_tours')->where('tour_id', $tour_id)->count();
            if ($check > 0) {
                DB::table('location_tours')->where('tour_id', $tour_id)->delete();
            }

            $params = [];
            foreach ($lats as $key => $lat) {
                $params[] = ['tour_id' => $tour_id, 'latitude' => $lat, 'longitude' => $longs[$key], 'name' => $name[$key]];
            }

            DB::table('location_tours')->insert($params);
    ...
    public function destroyProductFile($id)
    {
        return $this->deleteImage($this->product, $id, $path_name = 'file');
    }
    public function destroyProductFile2($id)
    {
        return $this->deleteImage($this->product, $id, $path_name = 'file2');
    }
    ...
    public function tabStore(Request $request)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('admin.product.tab', ['product_id' => $request->input('product_id')])->with("alert", "Thêm điểm đến thành công");
        } catch (\Exception $exception) {
            DB::rollBack();
            Log::error('message' . $exception->getMessage() . 'line :' . $exception->getLine());
            return redirect()->route('admin.product.tab', ['product_id' => $request->input('product_id')])->with("error", "Thêm điểm đến không thành công");
        }
    }

    public function editProductTab($id, Request $request)
    {
    ...
    public function updateProductTab(Request $request, $id)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('admin.product.tab', ['product_id' => $request->input('product_id')])->with("alert", "Sửa điểm đến thành công");
        } catch (\Exception $exception) {
            //throw $th;
            DB::rollBack();
            Log::error('message' . $exception->getMessage() . 'line :' . $exception->getLine());
            return redirect()->route('admin.product.tab', ['product_id' => $request->input('product_id')])->with("error", "Sửa điểm đến không thành công");
        }
    }
